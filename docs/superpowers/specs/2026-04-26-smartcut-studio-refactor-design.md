...
- `func setPauseThreshold(_ seconds: TimeInterval)` — clamps to `1.0...3.0`, updates `pauseThreshold`, calls `PauseDetector.detect(in: cachedSegments, totalDuration: cachedDuration, threshold: seconds)` (existing static API on the `PauseDetector` enum at `Services/PauseDetector.swift:7`) to produce a fresh `[PauseEdit]` (every emitted pause has `isEnabled: true` — see `PauseDetector.swift:14,23,30`). **Preserves user toggles via exact id-string match**: walk the new array, look up each `id` in the prior `editList.pauses`, copy the prior `isEnabled` if found, otherwise keep the detector's default `true`. Existing `PauseEdit.id` is `"pause@\(timeRange.lowerBound)"` and `PauseDetector` is deterministic for the same cached segments, so surviving pauses keep their identical id. Replaces `editList.pauses` synchronously. No-op when `cachedSegments.isEmpty`. **Does NOT fire a haptic itself** — haptic responsibility lives in the caller (`PauseControlRow.onChange`, see §4.7), keeping the VM haptic-free.
...
- Add two **optional** parameters to `SegmentedPill`: `selectedTint: PillButtonStyle.Tint = .ai` and `unselectedTint: PillButtonStyle.Tint = .accent`. Existing call-sites (Cleaning Lab tabs picker, Smart Cut/AI Denoise switcher) pick up the defaults — no behavior change. Studio's Original/Cleaned picker passes `selectedTint: .accent, unselectedTint: .accent` so selected = filled Deep Indigo, unselected = outline Deep Indigo.
